...
//===========================================================================
function DCYWC__generate_new_DinastyCoin_payment_id ($dcywc_settings=false, $order_info)
{
  global $wpdb;
  ...
  $new_dcy_payment_id = $wallet_api->makePaymentId();
  ...
  $dcy_address = $dcywc_settings['address'];
  ...
  $address_request_array['dcy_address'] = $dcy_address;
  $address_request_array['dcy_payment_id'] = $new_dcy_payment_id;
  ...
  $address_request_array['api_timeout'] = $dcywc_settings['blockchain_api_timeout_secs'];
  $ret_info_array = DCYWC__getreceivedbyaddress_info ($address_request_array, $dcywc_settings);
  ...
  $address_meta_serialized = DCYWC_serialize_address_meta ($address_meta);
  //$remote_addr  = $order_info['requested_by_ip'];

  // Insert newly generated address into DB
  $query = "INSERT INTO `$dcy_payments_table_name` (`dcy_address`, `dcy_payment_id`, `origin_id`, `index_in_wallet`, `status`, `total_received_funds`, `received_funds_checked_at`, `assigned_at`, `address_meta`) VALUES ('$dcy_address', '$new_dcy_payment_id', 'none', '$next_key_index', '$status', '$funds_received', '$received_funds_checked_at_time', '$assigned_at_time', '$address_meta_serialized');";
  $ret_code = $wpdb->query($query);

  return $new_dcy_payment_id;
}
//===========================================================================

//===========================================================================
// Function makes sure that returned value is valid array
function DCYWC_unserialize_address_meta ($flat_address_meta)
{
...
function DCYWC_serialize_address_meta ($address_meta_arr)
{
   return DCYWC__safe_string_escape(serialize($address_meta_arr));
}
//===========================================================================
...
function DCYWC__getreceivedbyaddress_info ($address_request_array, $dcywc_settings=false)
{
	if (!$dcywc_settings)
  	$dcywc_settings = DCYWC__get_settings ();

  $dcy_address            = $address_request_array['dcy_address'];
	$dcy_payment_id         = $address_request_array['dcy_payment_id'];
  ...
  $fnw = New ForkNoteWalletd("http://127.0.0.1:8071");
  $status = $fnw->getStatus();

  $t = $fnw->getTransactions( $status["blockCount"] - 50000, false, 50000, $dcy_payment_id, [$dcy_address]);
  // print_r( $t );
...
//===========================================================================
// Returns:
//    success: number of currency units (dollars, etc...) would take to convert to 1 DinastyCoin, ex: "15.32476".
//    failure: false
//
...
function DCYWC__get_exchange_rate_per_DinastyCoin ($currency_code, $rate_retrieval_method = 'getfirst', $get_ticker_string=false)
{
   if ($currency_code == 'DCY')
      return "1.00";   // 1:1

	$dcywc_settings = DCYWC__get_settings ();
  $exchange_rate_type = $dcywc_settings['exchange_rate_type'];
  $exchange_multiplier = $dcywc_settings['exchange_multiplier'];
  if (!$exchange_multiplier)
	...
	$ticker_string = "<span style='color:#222;'>According to your settings (including multiplier), current calculated rate for 1 DinastyCoin (in {$currency_code})={{{EXCHANGE_RATE}}}</span>";
	...
	$this_currency_info = @$dcywc_settings['exchange_rates'][$currency_code][$requested_cache_method_type];
	if ($this_currency_info && isset($this_currency_info['time-last-checked']))
	{
	  $delta = $current_time - $this_currency_info['time-last-checked'];
	  if ($delta < (@$dcywc_settings['cache_exchange_rates_for_minutes'] * 60))
	  {
  ...
  $exchange_rate = DCYWC__get_exchange_rate_from_cryptocompare($currency_code, $exchange_rate_type, $dcywc_settings); 

  // Save new currency exchange rate info in cache
  DCYWC__update_exchange_rate_cache ($currency_code, $requested_cache_method_type, $exchange_rate);
...
//===========================================================================
function DCYWC__function_not_exists ($fname) { return !function_exists($fname); }
//===========================================================================

//===========================================================================
function DCYWC__update_exchange_rate_cache ($currency_code, $requested_cache_method_type, $exchange_rate)
{
  // Save new currency exchange rate info in cache
  $dcywc_settings = DCYWC__get_settings ();   // Re-get settings in case other piece updated something while we were pulling exchange rate API's...
  $dcywc_settings['exchange_rates'][$currency_code][$requested_cache_method_type]['time-last-checked'] = time();
  $dcywc_settings['exchange_rates'][$currency_code][$requested_cache_method_type]['exchange_rate'] = $exchange_rate;
  DCYWC__update_settings ($dcywc_settings);
...
function DCYWC__get_exchange_rate_from_cryptocompare ($currency_code, $rate_type, $dcywc_settings)
{
 $source_url = "https://min-api.cryptocompare.com/data/price?fsym=DCY&tsyms=" . $currency_code;
 $result = @DCYWC__file_get_contents ($source_url, false, $dcywc_settings['exchange_rate_api_timeout_secs']);
...
*/
function DCYWC__file_get_contents ($url, $return_content_on_error=false, $timeout=60, $user_agent=FALSE, $is_post=false, $post_data="")
{
		...
		foreach ($post_data as $k => $v)
			{
				...
				if (is_object($safetied))
					$safetied = DCYWC__object_to_array($safetied);
				if (is_array($safetied))
				{
					$safetied = serialize($safetied);
					$safetied = $p . str_replace('=', '_', DCYWC__base64_encode($safetied));
...
//===========================================================================
function DCYWC__object_to_array ($object)
{
	if (!is_object($object) && !is_array($object))
    return $object;
  return array_map('DCYWC__object_to_array', (array) $object);
}
//===========================================================================

//===========================================================================
// Credits: http://www.php.net/manual/en/function.mysql-real-escape-string.php#100854
function DCYWC__safe_string_escape ($str="")
{
   $len=strlen($str);
   $escapeCount=0;
   $targetString='';
...
//===========================================================================
// Syntax:
//    DCYWC__log_event (__FILE__, __LINE__, "Hi!");
//    DCYWC__log_event (__FILE__, __LINE__, "Hi!", "/..");
//    DCYWC__log_event (__FILE__, __LINE__, "Hi!", "", "another_log.php");
function DCYWC__log_event ($filename, $linenum, $message, $prepend_path="", $log_file_name='__log.php')
{
   $log_filename   = dirname(__FILE__) . $prepend_path . '/' . $log_file_name;
   $logfile_header = "<?php exit(':-)'); ?>\n" . '/* =============== DinastyCoinWC LOG file =============== */' . "\r\n";
   $logfile_tail   = "\r\nEND";
   ...
   if ($fhandle)
      {
      @fwrite ($fhandle, "\r\n// " . $_SERVER['REMOTE_ADDR'] . '(' . $_SERVER['REMOTE_PORT'] . ')' . ' -> ' . date("Y-m-d, G:i:s T") . "|" . DCYWC_VERSION . "/" . DCYWC_EDITION . "|$filename($linenum)|: " . $message . $logfile_tail);
      @fclose ($fhandle);
      }
}
//===========================================================================
...
//===========================================================================
function DCYWC__send_email ($email_to, $email_from, $subject, $plain_body)
{
   $message = "
   <html>
   <head>
...
//===========================================================================
function DCYWC__is_gateway_valid_for_use (&$ret_reason_message=NULL)
{
  $valid = true;
  $dcywc_settings = DCYWC__get_settings ();
...
////   'service_provider'                     =>  'local_wallet',    // 'blockchain_info'
  ...
  //----------------------------------
  // Validate settings
  if ($dcywc_settings['service_provider']=='local_wallet')
  {         
          $dcywc_settings = DCYWC__get_settings();
...
function DCYWC__base64_decode($input)
{
...
function DCYWC__base64_encode($data)
{
    ...
    $o1 = DCYWC_charCodeAt($data, $i++);
    $o2 = DCYWC_charCodeAt($data, $i++);
    $o3 = DCYWC_charCodeAt($data, $i++);
    $bits = $o1 << 16 | $o2 << 8 | $o3;
    $h1 = $bits >> 18 & 0x3f;
    $h2 = $bits >> 12 & 0x3f;
    $h3 = $bits >> 6 & 0x3f;
    ...
    $tmp_arr[$ac++] = DCYWC_charAt($b64, $h1).DCYWC_charAt($b64, $h2).DCYWC_charAt($b64, $h3).DCYWC_charAt($b64, $h4);
    } while ($i < strlen($data));
    $enc = implode($tmp_arr, '');
    $r = (strlen($data) % 3);
...
function DCYWC_charCodeAt($data, $char) {
    return ord(substr($data, $char, 1));
}

function DCYWC_charAt($data, $char) {
